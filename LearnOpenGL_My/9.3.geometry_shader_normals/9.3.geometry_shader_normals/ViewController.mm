...
- (void)configOpenGLEnvironment
{
    ...
    {
        NSURL *shader_vs = [self findResourceWithName:@"9.3.normal_visualization.vs"];
        NSAssert(shader_vs, @"Cannot to find out a shader");
        
        NSURL *shader_fs = [self findResourceWithName:@"9.3.normal_visualization.fs"];
        NSAssert(shader_fs, @"Cannot to find out a shader");
        
        NSURL *shader_gs = [self findResourceWithName:@"9.3.normal_visualization.gs"];
        NSAssert(shader_fs, @"Cannot to find out a shader");
    ...
    {
        NSURL *shader_vs = [self findResourceWithName:@"9.3.default.vs"];
        NSAssert(shader_vs, @"Cannot to find out a shader");
        
        NSURL *shader_fs = [self findResourceWithName:@"9.3.default.fs"];
        NSAssert(shader_fs, @"Cannot to find out a shader");
    ...
    NSURL *nanosuit = [self findResourceWithName:@"objects/nanosuit/nanosuit.obj"];
    NSAssert(nanosuit, @"Cannot to find out an object");
    
    _nanosuit = new Model([nanosuit fileSystemRepresentation]);
    
    _camera = new Camera(glm::vec3(0.0f, 20.0f, 30.0f));
}
...
- (void)renderForTime:(CVTimeStamp)time
{
    ...
    if (NULL != _shader && NULL != _nanosuit)
    {
