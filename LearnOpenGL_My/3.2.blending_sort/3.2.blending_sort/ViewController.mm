...
- (void)configOpenGLEnvironment
{
    ...
    NSURL *shader_vs = [self findResourceWithName:@"3.2.blending.vs"];
    NSAssert(shader_vs, @"Cannot to find out a shader");
    
    NSURL *shader_fs = [self findResourceWithName:@"3.2.blending.fs"];
    ...
    _transparentTexture = [self loadTexture:[[self findResourceWithName:@"textures/window.png"] fileSystemRepresentation]];
...
- (void)renderForTime:(CVTimeStamp)time
{
    ...
    if (_shader)
    {
        ...
        {
            float distance = glm::length(_camera->Position - windows[i]);
